...
async function clean(options: MitosisConfig) {
  const patterns = options.targets.map(
    (target) =>
      `${options.dest}/${options.getTargetPath({ target })}/${options.files}`
  );
...
const parseSvelteComponent = async ({
  path,
  file,
}: {
  path: string;
...
const getMitosisComponentJSONs = async (
  options: MitosisConfig
): Promise<ParsedMitosisJson[]> => {
  const paths = getFiles({
    files: options.files,
    exclude: options.exclude,
  }).filter(checkIsMitosisComponentFilePath);
  return Promise.all(
...
async function buildAndOutputComponentFiles({
  ...
  const shouldOutputTypescript = checkShouldOutputTypeScript({
    options,
    target,
  });

  const output = files.map(
    async ({ path, typescriptMitosisJson, javascriptMitosisJson }) => {
      const outputFilePath = getComponentOutputFileName({
        target,
        path,
        options,
      });
      ...
      try {
        const component = shouldOutputTypescript
          ? typescriptMitosisJson
          : javascriptMitosisJson;

        transpiled =
          overrideFile ??
          generator(options.options[target])({ path, component });
        debugTarget(
...
const getNonComponentFileExtension = flow(
  checkShouldOutputTypeScript,
  (shouldOutputTypeScript) => (shouldOutputTypeScript ? '.ts' : '.js')
);
...
async function buildNonComponentFiles(args: TargetContextWithConfig) {
  ...
  const nonComponentFiles = getFiles({
    files: options.files,
    exclude: options.exclude,
  }).filter((file) => file.endsWith('.ts') || file.endsWith('.js'));

  return await Promise.all(
    nonComponentFiles.map(
      async (path): Promise<{ path: string; output: string }> => {
        /**
         * Try to find override file.
         * NOTE: we use the default `getTargetPath` even if a user-provided alternative is given. That's because the
         * user-provided alternative is only for the output path, not the override input path.
